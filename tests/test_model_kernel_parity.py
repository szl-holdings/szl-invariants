...
def test_trained_surrogate_disclosure_is_present_on_both_surfaces() -> None:
    for path in (MODEL_SOURCE, KERNEL_SOURCE):
        ...
        assert "ALSO ships a REAL trained companion model" in docstring
        assert "model.joblib, sklearn" in docstring
        assert provenance["trained_weights_present"] is True
